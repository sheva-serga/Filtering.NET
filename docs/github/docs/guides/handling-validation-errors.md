...
- The codes the validator produces are `UnknownField`, `OperatorNotAllowed`, `InvalidValueType`, `InterceptorRejected`, `NotSortable`, `InvalidSortDirection`, `PageInvalid`, `PageSizeTooLarge`, `PageSizeInvalid`, `NestingTooDeep`, `TooManyConditions`, `GroupEmpty`, and `InvalidNodeShape`. UI translations and per-error help links should switch on these codes, not on the human-readable message. A malformed value arrives as `InvalidValueType`, and an empty `in` array validates clean and matches nothing.
- `FilterValidationCode` is written to JSON as the member name (the enum carries a `JsonStringEnumConverter`); its numeric values are pinned for callers that stored them.
