...
- **`Filtering.Net.Generator`** ships the Roslyn incremental source generator and its 30-rule analyzer (`FN0001`–`FN0022` errors, `FN1001`–`FN1008` warnings). It is analyzer-only — no runtime DLL — and is consumed as an analyzer reference at compile time.
